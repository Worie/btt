...
export default abstract class Utilities {
   ...
   * @param baseUrl
   */
  public buildFullUrl(action: string, params: string, baseUrl: string): string {
   ...
   * @param key
   */
  public translateObjectKeysToBttNotation(object: Partial<Types.AppPayload> | Partial<Types.BttPayload>) {
   ...
   * @param string
   */
  public simpleCase(value: string) {
    return value
      .replace(/-/g, '')
      .replace(/_/g, '')
      .toLocaleLowerCase();
  ...
  public mapClassNameToMethodName(theClass: Types.Class<BaseAction>) {
    if (theClass.name[0] !== 'A' && theClass.prototype instanceof BaseAction) {
